...
module.exports = function blowUpFX ( entity ) {
  ...
  game.globals.renderCallbacks.push(function ( screen ) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      pixels.push(new Pixel(0,1,1,1,[coords[0] + i * point.direction[0], coords[1]]).setColor(color, factor));
      pixels.push(new Pixel(0,1,1,1,[coords[0] - i * point.direction[0], coords[1]]).setColor(color, factor));
      pixels.push(new Pixel(0,1,1,1,[coords[0], coords[1] + i * point.direction[1]]).setColor(color, factor));
      pixels.push(new Pixel(0,1,1,1,[coords[0], coords[1] - i * point.direction[1]]).setColor(color, factor));
    }
